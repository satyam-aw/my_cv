...
Computer Science researcher specializing in the intersection of \textbf{Autonomous Decision-Making}, \textbf{Spatial Perception}, and \textbf{Robot Learning} within Embodied Systems, with a core focus on engineering verifiably safe, constraint-bound execution loops.
...
% ==================== SKILLS ====================
\section*{Skills}
\noindent \textbf{Programming \& Simulation:} Python, C++, MATLAB, Kotlin, Java, Gazebo, Rviz, Unity, Git \\
\noindent \textbf{Robotics, Vision \& AI Frameworks:} ROS/ROS2, OpenCV, PyTorch, LangChain, Pydantic, ARCore/iOS ARKit \\
\noindent \textbf{Core Methodologies:} State Estimation (EKF), Control Theory (MPC), Kinematic Modeling, Spatial Perception \\
\noindent \textbf{Fields of Interest:} Autonomous Decision-Making, Spatial Perception, Robot Learning
...
\begin{itemize}
    \item Developed a hardware-agnostic navigation stack in ROS, executing an Extended Kalman Filter (EKF) to fuse multi-modal sensor streams (LiDAR, IMU, GPS) for zero-shot \textbf{Sim2Real transfer}.
    \item Implemented real-time visual obstacle segmentation using a custom U-Net architecture alongside a hybrid motion planner combining global ($A^*$/Dijkstra) paths with a Dynamic Window Approach (DWA) for local collision avoidance.
    \item Formulated a decentralized swarm coordination network utilizing peer-to-peer relative pose estimation via AprilTags over an ad-hoc XBee mesh topology.
\end{itemize}
